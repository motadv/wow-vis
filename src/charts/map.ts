...
export function initMap(container: HTMLElement, m: DungeonManifest): void {
  ...
  const zoom = d3
    ...
    .scaleExtent([0.4, 5])
    .on("zoom", (event) =>
      innerG.attr("transform", event.transform.toString()),
    );
  ...
  svg.on("click", function (event: MouseEvent) {
    ...
    const [x, y] = d3.pointer(event, innerG.node());
    const coords = `mapX: ${Math.round(x)}, mapY: ${Math.round(y)},`;
...
function renderNodes(): void {
  ...
  const enter = node
    ...
    .on("mouseenter", function (_event, d) {
      ...
      const stats = document.createTextNode(
        ` · ${d.era} · max ${vol?.max_key ?? "—"} · n=${vol?.entry_count ?? 0}`,
      );
  ...
  enter
    ...
    .attr("stroke", (d) =>
      d.id === selectedDungeon ? "#ffffff" : "transparent",
    )
    .attr("stroke-width", 2)
    .attr("opacity", (d) => {
